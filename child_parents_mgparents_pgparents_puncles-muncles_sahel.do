cd"C:\Users\ycompaore\Desktop\Redaction thèse Yacou\Après confirmation\Paper 1\data\Final data\muncles"
local originals "C:\Users\ycompaore\Desktop\Redaction thèse Yacou\Après confirmation\Paper 1\data\Final data\muncles" // OR WHATEVER THE RIGHT DIRECTORY IS.
local files: dir "`originals'" files "child_muncle_*.dta" // NOTE : after C
...
use child_network_pa_paunt.dta,clear
tempfile building
...
keep if _merge==2
rename _merge mergeu`w'
append using `building', gen(appendmu_`w')
...
recode  EventC* (.=9) if appendmu_`w'==0
recode resid* (.=0) if appendmu_`w'==0
...
keep if _merge==2
rename _merge mergemu1_`w'
append using `"`f'"',gen(appendmu__`w')
...
recode  EventC* (.=9) if appendmu__`w'==0
recode resid* (.=0) if   appendmu__`w'==0
...
save child_muncle`w'_sahel_bis, replace
...
clear
capture erase building_a.dta
tmerge  concat_IndividualId child_network_sahel_bis(EventDate) child_muncle`w'_sahel_bis(EventDate1) ///
...
replace EventDatemuncle`w' = 18 if _File==2
capture drop dup_e
sort concat_IndividualId EventDate EventCode
...
label data "file_muncles, long format"
save `"`dir1'/child_network_pa_paunt_muncle.dta"', replace
save `"`dir2'/child_network_pa_paunt_muncle.dta"', replace
