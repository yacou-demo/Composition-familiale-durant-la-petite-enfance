clear
import excel RESIDENT.xlsx, sheet("RESIDENT") firstrow
save "T_resident",replace

br RESI_CP_resident RESI_CE_pere RESI_CE_mere
count if RESI_CE_pere==.
count if RESI_CE_mere==.
*Je préfère remplacer les valeurs manquantes par missing
replace RESI_CE_pere=. if RESI_CE_pere==-1
replace RESI_CE_mere=. if RESI_CE_mere==-1

rename RESI_CP_resident IndividualId
rename RESI_CE_pere fatherId
rename RESI_CE_mere motherId
rename RESI_dt_nais DoB
rename RESI_cd_sexe Sex
drop if motherId==fatherId
drop if IndividualId==motherId
drop if  IndividualId==fatherId
save individual_parents_bis,replace
...
foreach var of varlist IndividualId fatherId motherId Sex  {
...
use individual_parents_bis,clear
...
merge m:1 IndividualId using individual_parents_bis, keep(1 3) keepus(DoB Sex)
...
if SexMO=="M", firstrow(variables) sh("mother Sex=M") 
...
if SexFA=="F", firstrow(variables) sh("father Sex="F"") 
drop _merge
...
rename motherId IndividualId
merge m:1 IndividualId using individual_parents_bis,keep(1 3) keepus(motherId)gen(mergeGMM)
...
merge m:1 IndividualId using individual_parents_bis,  keep(1 3) keepus(DoB Sex)gen(mergeMGM)
...
if  SexMGM=="M", firstrow(variables) sh("mgmother Sex="M"") 
*drop _merge
...
rename IndividualId ChildId
rename fatherId IndividualId
merge m:1 IndividualId using individual_parents_bis,keep(1 3) keepus(fatherId)gen(mergeGPP)
 *fatherId pgfatherId
...
merge m:1 IndividualId using individual_parents_bis,  keep(1 3) keepus(DoB Sex)gen(mergePGF)
...
if  SexPGF=="F", firstrow(variables) sh("pgfather Sex="F"") 
...
rename IndividualId ChildId
rename motherId IndividualId
merge m:1 IndividualId using individual_parents_bis,keep(1 3) keepus(fatherId)gen(mergeGPM)
 *fatherId mgfatherId
...
merge m:1 IndividualId using individual_parents_bis,  keep(1 3) keepus(DoB Sex)gen(mergeMGF)
...
if  SexMGF=="F", firstrow(variables) sh("mgfatherId Sex="F"") 
...
rename IndividualId ChildId
rename fatherId IndividualId
merge m:1 IndividualId using individual_parents_bis,keep(1 3) keepus(motherId)gen(mergeGMP)
...
merge m:1 IndividualId using individual_parents_bis,  keep(1 3) keepus(DoB Sex)gen(mergePGM)
...
if  SexPGM=="M", firstrow(variables) sh("pgmotherId Sex="F"") 
...
format DoB  %td
save family_Niakhar_all_var,replace
...
sort IndividualId 
merge 1:m IndividualId using socialgroup ,keepus(HouseholdId)gen(merge_sgp)
rename HouseholdId socialgpId 
drop if socialgpId==""
save familly_Niakhar_final, replace


**Oncles, cousins et tantes

use resid_complet,clear
keep IndividualId HouseholdId Compound 
tostring IndividualId,replace
tostring HouseholdId,replace
save socialgroup,replace

use family_Niakhar_all_var,clear
sort IndividualId 
merge 1:m IndividualId using socialgroup ,keepus(HouseholdId Compound)gen(merge_sgp)
rename HouseholdId socialgpId 
drop if socialgpId==""
...
use familly_Niakhar_final,clear
...
save family_Niakhar_puncle,replace
...
format DoB_paunt* %td
save family_Niakhar_paunt,replace
...
save family_Niakhar_muncle,replace
...
format DoB_maunt* %td
save family_Niakhar_maunt,replace
...
duplicates drop IndividualId,force
sort   IndividualId 
merge 1:1  IndividualId using family_Niakhar_puncle,gen(mergep)
sort  IndividualId
merge 1:1  IndividualId using family_Niakhar_paunt,gen(mergea)
sort  IndividualId
merge 1:1  IndividualId using family_Niakhar_muncle,gen(mergem)
sort  IndividualId
merge 1:1  IndividualId using family_Niakhar_maunt,gen(mergema)
drop merge*
...
save familly_Niakhar_fnetwork, replace
