*-------------------------------------------------------------------------------
**Child - Parents - Maternal grand parents - Paternal grand mother**
*-------------------------------------------------------------------------------

use child_parents_mgparents_sahel_final,clear
...
save child_parents_mgparents_sahel_unique,replace

use  child_pgmother_sahel_clean.dta,clear
...
save child_pgmother_sahel_unique,replace



use child_parents_mgparents_sahel_unique,clear
sort concat_IndividualId
merge 1:1 hdss  concat_IndividualId using child_pgmother_sahel_unique
keep if _merge==2
rename _merge merge_addpgm
append using child_parents_mgparents_sahel_final, gen(appendpgm)
...
foreach var of varlist EventCode EventCodeMO EventCodeFA EventCodeMGM EventCodeMGF {  
recode  `var' .=9 if appendpgm==0
}
recode residenceMO (.=0) if appendpgm==0
recode residenceFA (.=0) if appendpgm==0
recode residenceMGM (.=0) if appendpgm==0
recode residenceMGF (.=0) if appendpgm==0
...
save child_parents_mgparents_sahel_final_bis,replace



use child_pgmother_sahel_unique,clear
sort concat_IndividualId
merge 1:1 hdss  concat_IndividualId using child_parents_mgparents_sahel_unique
keep if _merge==2
rename _merge merge_addppgm
append using child_pgmother_sahel_clean,gen(appendppgm)
...
foreach var of varlist EventCode EventCodePGM   {  
recode  `var' .=9 if appendppgm==0
}
recode residencePGM (.=0) if appendppgm==0
...
save child_pgmother_sahel_bis, replace


/***********************TMERGE*******************************/
capture erase child_parents_mgparents_pgmother_sahel.dta
clear
capture erase child_parents_mgparents_pgmother_sahel.dta
tmerge  concat_IndividualId child_parents_mgparents_sahel_final_bis(EventDate) child_pgmother_sahel_bis(EventDate1) ///
		child_parents_mgparents_pgmother_sahel(EventDate_final)
...
sort concat_IndividualId EventDate EventCode
rename _File child_parents_mgparents_pgmother
...
save child_parents_mgparents_pgmother_sahel_final,replace
