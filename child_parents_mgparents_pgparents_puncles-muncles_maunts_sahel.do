...
local dir2 "C:\Users\ycompaore\Desktop\Redaction thèse Yacou\Après confirmation\Paper 1\data analysis"

use child_network_pa_paunt_muncle.dta,clear
tempfile building
...
keep if _merge==2
rename _merge mergema`w'
append using `building', gen(appendma_`w')
...
recode  EventC* (.=9) if appendma_`w'==0
recode resid* (.=0) if appendma_`w'==0
...
keep if _merge==2
rename _merge mergema1_`w'
append using `"`f'"',gen(appendma__`w')
...
recode  EventC* (.=9) if appendma__`w'==0
recode resid* (.=0) if   appendma__`w'==0
...
save child_maunt`w'_sahel_bis, replace
...
clear
capture erase building_a.dta
tmerge  concat_IndividualId child_network_sahel_bis(EventDate) child_maunt`w'_sahel_bis(EventDate1) ///
...
replace EventDatemaunt`w' = 18 if _File==2
capture drop dup_e
sort concat_IndividualId EventDate EventCode
...
label data "file_maunts, long format"
save `"`dir1'/child_network_pa_paunt_muncle_maunt.dta"', replace
save `"`dir2'/child_network_pa_paunt_muncle_maunt.dta"', replace
