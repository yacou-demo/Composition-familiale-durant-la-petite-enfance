...
/****************CREATING Child dataset with pauntid ***************/
...
drop if paunt_ego`i'=="---------"
duplicates drop   concat_IndividualId  paunt_ego`i',force
...
keep concat_IndividualId pauntid`i' DoB_paunt`i' hdss  socialgpId 
*ds,has(type pauntid)
keep if pauntid`i'!=" "
keep if pauntid`i'!=""
*0 deleted
...
* import pauntid in the core residency file
*modif
merge m:1 concat_IndividualId  using paunt`i'_childID_sahel.dta
sort concat_IndividualId EventDate EventCode
...
/****************Create paunt_children data*************/
use child_sahel_paunt`i', clear
codebook concat_IndividualId
**Uniqued IndiId  =130,051
codebook PauntId`i'
**Uniqued PauntId =50,179
keep concat_IndividualId PauntId`i' DoB hdss 

duplicates drop concat_IndividualId  PauntId`i' DoB ,force
rename concat_IndividualId ChildId
rename  PauntId`i' concat_IndividualId
...
***********Merge***Childpaunt data with paunt Residency data***********/
use residency_sahel, clear
rename socialgpid socialgpId
merge m:1   concat_IndividualId using paunt`i'_children_sahel.dta
*drop DoB*
...
codebook concat_IndividualId if paunt`i'data!=1
*122,976
...
* to check that all EventDate end up 1 Jan 2013 except for non-resident paunts
capture drop maxEventDate
...
bys concat_IndividualId: replace DoB_paunt`i' = DoB_paunt`i'[_n-1] if missing(DoB_paunt`i') & _n > 1 
bys concat_IndividualId: replace DoB_paunt`i' = DoB_paunt`i'[_N]
...
rename concat_IndividualId PauntId`i'

rename ChildId concat_IndividualId
recode res_paunt`i' .=0

 
order concat_IndividualId child_rank PauntId`i' EventDatepaunt`i' EventCodepaunt`i' 
sort concat_IndividualId EventDatepaunt`i' EventCodepaunt`i' 


count if EventDatepaunt`i'==.

bys concat_IndividualId : replace last_record_date=21658  if EventDatepaunt`i'==. & hdss=="GM011"
bys concat_IndividualId : replace last_record_date=21185  if EventDatepaunt`i'==. & hdss=="BF041"
bys concat_IndividualId : replace last_record_date=21425  if EventDatepaunt`i'==. & hdss=="BF021"
bys concat_IndividualId : replace last_record_date=20454  if EventDatepaunt`i'==. & hdss=="SN011"
bys concat_IndividualId : replace last_record_date=21185  if EventDatepaunt`i'==. & hdss=="SN012"
...
clear
capture erase child_paunt`i'_sahel.dta
tmerge concat_IndividualId child_sahel_paunt`i'(EventDate) childpaunt`i'_sahel(EventDatepaunt`i') ///
...
use child_paunt_`i',clear


*for sgp_paunt
sort hdss concat_IndividualId EventDate 
bys hdss  concat_IndividualId : replace EventCodepaunt`i'=6 if EventCodepaunt`i'[_n-1] ==5 & EventCodepaunt`i'==9 & EventDate==EventDate[_n-1]

sort hdss concat_IndividualId EventDate
bys hdss  concat_IndividualId : replace EventCodepaunt`i'=5 if EventCodepaunt`i'[_n-1] ==6 & EventCodepaunt`i'==9 & EventDate==EventDate[_n-1]
*123157 naissances
sort hdss concat_IndividualId EventDate 
bys hdss  concat_IndividualId : replace EventCodepaunt`i'=6 if EventCodepaunt`i'[_n+1] ==5 & EventCodepaunt`i'==9 & EventDate==EventDate[_n+1]
...
sort hdss concat_IndividualId EventDate 
bys hdss  concat_IndividualId : replace sgp_paunt`i'=sgp_paunt`i'[_n-2] if EventCodepaunt`i'==9 & EventCodepaunt`i'[_n-1]==9 
sort hdss concat_IndividualId EventDate EventCode

sort hdss concat_IndividualId EventDate 
bys hdss  concat_IndividualId : replace sgp_paunt`i'=sgp_paunt`i'[_N] if EventCodepaunt`i'==9 & EventCodepaunt`i'[_N]==9 & _n==_N-1
sort hdss concat_IndividualId EventDate EventCode
...
capture drop dup_e
sort hdss concat_IndividualId EventDate EventCode
 bys hdss concat_IndividualId EventDate EventCode EventCodepaunt`i' sgp_paunt`i': gen dup_e= cond(_N==1,0,_n)
