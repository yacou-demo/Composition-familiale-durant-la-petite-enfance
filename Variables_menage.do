...
bys IndividualID  : replace EventCode=4 if EventCode==5 &  socialgpid==socialgpid[_n+1]
*/
...
sort IndividualID EventDate EventCode
qui by IndividualID: gen datebeg=cond(_n==1, DoB, EventDate[_n-1])
...
by hdss  (EventDate), sort: gen last_obs = (_n == _N)
gen double last_record_date = EventDate if last_obs==1
format last_record_date %td
bysort hdss (EventDate): replace last_record_date = last_record_date[_N]
sort IndividualID EventDate EventCode
...
*Date d'entrée en observation de chaque individuals
sort IndividualID EventDate
by IndividualID : gen min_line=cond(_N==1,1,_n)

gen min_date = EventDate if min_line==1
sort  IndividualID min_line
bys  IndividualID: replace min_date = min_date[_n-1] if missing(min_date) & _n > 1 
bys IndividualID min_line: replace min_date = min_date[_N] if missing(min_date)
bys IndividualID min_line: replace min_date = min_date[_N] if missing(min_date)
...
*Supprimer les lignes inutiles basées sur la date d début d'observations
drop if EventDate < min_date
...
save base_analyse_final_Sahel,replace
