...
drop if pgfatherId=="---------"
duplicates drop   concat_IndividualId  pgfatherId,force
...
*modif
keep concat_IndividualId pgfatherid DoBPGF hdss socialgpId
...
*modif
merge m:1 concat_IndividualId  using pgfather_childID_sahel.dta
sort concat_IndividualId EventDate EventCode
...
codebook pgFatherId
**Uniqued pgFatherId =103,728 
keep concat_IndividualId pgFatherId DoB hdss 
duplicates drop concat_IndividualId pgFatherId DoB ,force
rename concat_IndividualId ChildId
rename pgFatherId concat_IndividualId
...
use residency_sahel, clear
rename socialgpid socialgpId
merge m:1   concat_IndividualId using pgfather_children_sahel.dta
*drop DoB*
...
codebook concat_IndividualId if pgfatherdata!=1
*122,976
...
rename DoB DoBPGF
bys concat_IndividualId: replace DoBPGF = DoBPGF[_n-1] if missing(DoBPGF) & _n > 1 
bys concat_IndividualId: replace DoBPGF = DoBPGF[_N]
...
rename concat_IndividualId pgFatherId
*rename DoD DoDFA
rename ChildId concat_IndividualId
recode residencePGF .=0

 
order concat_IndividualId child_rank pgFatherId EventDatePGF EventCodePGF 
sort concat_IndividualId EventDatePGF EventCodePGF
...
clear
capture erase child_pgfather_sahel.dta
tmerge concat_IndividualId child_sahel_PGF(EventDate) childPGF_sahel(EventDatePGF) ///
...
*for socialgpidFA
sort hdss concat_IndividualId EventDate 
bys hdss  concat_IndividualId : replace EventCodePGF=6 if EventCodePGF[_n-1] ==5 & EventCodePGF==9 & EventDate==EventDate[_n-1]

sort hdss concat_IndividualId EventDate
bys hdss  concat_IndividualId : replace EventCodePGF=5 if EventCodePGF[_n-1] ==6 & EventCodePGF==9 & EventDate==EventDate[_n-1]
*123157 naissances
sort hdss concat_IndividualId EventDate 
bys hdss  concat_IndividualId : replace EventCodePGF=6 if EventCodePGF[_n+1] ==5 & EventCodePGF==9 & EventDate==EventDate[_n+1]
...
sort hdss concat_IndividualId EventDate 
bys hdss  concat_IndividualId : replace socialgpidPGF=socialgpidPGF[_n-2] if EventCodePGF==9 & EventCodePGF[_n-1]==9 
sort hdss concat_IndividualId EventDate EventCode

sort hdss concat_IndividualId EventDate 
bys hdss  concat_IndividualId : replace socialgpidPGF=socialgpidPGF[_N] if EventCodePGF==9 & EventCodePGF[_N]==9 & _n==_N-1
sort hdss concat_IndividualId EventDate EventCode

sort hdss concat_IndividualId EventDate 
capture drop dup_e
sort hdss concat_IndividualId EventDate EventCode
 bys hdss concat_IndividualId EventDate EventCode EventCodePGF socialgpidPGF: gen dup_e= cond(_N==1,0,_n)
