use  individual_parents.dta,clear

count if id_pere==""
count if id_mere==""

*Je préfère remplacer les valeurs manquantes par missing
replace id_pere="" if id_pere=="UNK"
replace id_mere="" if id_mere=="UNK"

*rename uuid uuid_child
rename extid IndividualId
rename  id_pere fatherId
rename  id_mere motherId
rename dob_mere DoB_mother
rename dob_pere DoB_father
...
label var mgfatherId "Maternal Grand Father ID"
...
save family_Nanoro_all_var,replace

keep IndividualId DoB Sex  fatherId DoBFA motherId DoBMO mgmotherId DoBMGM ///
SexMGM mgfatherId DoBMGF SexMGF pgfatherId DoBPGF SexPGF pgmotherId DoBPGM SexPGM individual_uuid

save family_Nanoro, replace
...
merge 1:m IndividualId using residency_Nanoro_final ,keepus(socialgpid)gen(merge_sgp)
...
save familly_Nanoro_final, replace

**Oncles, cousins et tantes


use familly_Nanoro_final,clear
duplicates drop socialgpId IndividualId,force
...
save family_Nanoro_puncle,replace
...
format DoB_paunt* %td
save family_Nanoro_paunt,replace
...
save family_Nanoro_muncle,replace
...
format DoB_maunt* %td
save family_Nanoro_maunt,replace
...
sort   IndividualId 
duplicates drop IndividualId,force
merge 1:1  IndividualId using family_Nanoro_puncle,gen(mergep)
sort  IndividualId
merge 1:1  IndividualId using family_Nanoro_paunt,gen(mergea)
sort  IndividualId
merge 1:1  IndividualId using family_Nanoro_muncle,gen(mergem)
sort  IndividualId
merge 1:1  IndividualId using family_Nanoro_maunt,gen(mergema)
drop merge*
...
save familly_Nanoro_fnetwork, replace
