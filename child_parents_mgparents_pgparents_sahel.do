*-------------------------------------------------------------------------------
**Child - Parents - Maternal grand parents - Paternal grand parents**
*-------------------------------------------------------------------------------

local dir1 "C:\Users\ycompaore\Desktop\Redaction thèse Yacou\Après confirmation\Paper 1\data\Final data\puncles"
use child_parents_mgparents_pgmother_sahel_final,clear
...
save child_parents_mgparents_pgmother_sahel_unique,replace

use  child_pgfather_sahel_clean.dta,clear
...
save child_pgfather_sahel_unique,replace

use child_parents_mgparents_pgmother_sahel_unique,clear
sort concat_IndividualId
merge 1:1 hdss  concat_IndividualId using child_pgfather_sahel_unique
keep if _merge==2
rename _merge merge_addpgf
append using child_parents_mgparents_pgmother_sahel_final, gen(appendpgf)
...
foreach var of varlist EventCode EventCodeMO EventCodeFA EventCodeMGM EventCodeMGF EventCodePGM{  
recode  `var' .=9 if appendpgf==0
}
recode residenceMO (.=0) if appendpgf==0
recode residenceFA (.=0) if appendpgf==0
recode residenceMGM (.=0) if appendpgf==0
recode residenceMGF (.=0) if appendpgf==0
recode residencePGM (.=0) if appendpgf==0
...
save child_parents_mgparents_pgmother_sahel_final_bis,replace



use child_pgfather_sahel_unique,clear
sort concat_IndividualId
merge 1:1 hdss  concat_IndividualId using child_parents_mgparents_pgmother_sahel_unique
keep if _merge==2
rename _merge merge_addppgf
append using child_pgfather_sahel_clean,gen(appendppgf)
...
foreach var of varlist EventCode EventCodePGF   {  
recode  `var' .=9 if appendppgf==0
}
recode residencePGF (.=0) if appendppgf==0
...
save child_pgfather_sahel_bis, replace


/***********************TMERGE*******************************/
capture erase child_parents_mgparents_pgparents_sahel.dta
clear
capture erase child_parents_mgparents_pgparents_sahel.dta
tmerge  concat_IndividualId child_parents_mgparents_pgmother_sahel_final_bis(EventDate) child_pgfather_sahel_bis(EventDate1) ///
		child_parents_mgparents_pgparents_sahel(EventDate_final)
...
sort concat_IndividualId EventDate EventCode
rename _File child_pa_mgpa_pgpa
		
drop if MotherId==""
replace  coresidFA =0 if FatherId==""
replace coresidMGM =0 if mgMotherId==""
replace coresidMGF =0 if mgFatherId==""
replace coresidPGM =0 if pgMotherId==""
replace coresidPGF =0 if pgMotherId==""

save child_parents_mgparents_pgparents_sahel_final,replace
save `"`dir1'/child_parents_mgparents_pgparents_sahel_final"', replace
