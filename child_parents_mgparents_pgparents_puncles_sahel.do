cd"C:\Users\ycompaore\Desktop\Redaction thèse Yacou\Après confirmation\Paper 1\data\Final data\puncles"
local originals "C:\Users\ycompaore\Desktop\Redaction thèse Yacou\Après confirmation\Paper 1\data\Final data\puncles" // OR WHATEVER THE RIGHT DIRECTORY IS.
local files: dir "`originals'" files "child_puncle_*.dta" // NOTE : after C
...
local dir2 "C:\Users\ycompaore\Desktop\Redaction thèse Yacou\Après confirmation\Paper 1\data\Final data\paunts" // LOCALS ARE SAFER THAN GLOBALS



use child_parents_mgparents_pgparents_sahel_final,clear
tempfile building
save `building', emptyok

foreach f of local files {
	   
local w = strreverse(substr(strreverse(substr(`"`f'"', 14, .)), 5,.))

use `"`building'"',clear
sort concat_IndividualId EventDate EventCode
capture drop dup_a
quietly bys concat_IndividualId EventDate EventCode: gen dup_a = cond(_N==1,0,_n)
*drop if dup_a>1
...
append using `building', gen(append_`w')
...
recode  EventC* (.=9) if append_`w'==0
recode resid* (.=0) if append_`w'==0
...
keep if _merge==2
rename _merge merge_`w'
append using `"`f'"',gen(append__`w')
...
recode  EventC* (.=9) if append__`w'==0
recode resid* (.=0) if   append__`w'==0
...
save child_uncle`w'_sahel_bis, replace
...
clear
capture erase building_a.dta
tmerge  concat_IndividualId child_network_sahel_bis(EventDate) child_uncle`w'_sahel_bis(EventDate1) ///
...
replace EventCodepuncle`w' = 18 if _File==2
capture drop dup_e
sort concat_IndividualId EventDate EventCode
...
label data "file_puncles, long format"
save `"`dir1'/child_network_pu.dta"', replace
save `"`dir2'/child_network_pu.dta"', replace
